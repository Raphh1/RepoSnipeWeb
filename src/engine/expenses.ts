...
export function getDailyExpenseBreakdown(gs: GameState): { label: string; amount: number }[] {
  ...
  const baseMaint = gs.day <= 5 ? 20 : gs.day <= 15 ? 50 : gs.day <= 30 ? 80 : 120
  lines.push({ label: 'Maintenance de base', amount: baseMaint })

  const moduleCost = Object.values(gs.shipModules ?? {}).reduce((sum, lvl) => sum + (lvl ?? 0) * 15, 0)
  if (moduleCost > 0) lines.push({ label: 'Modules vaisseau', amount: moduleCost })

  const station = getStation(gs.currentStation)
  if (station) {
    const dangerCost = [0, 30, 60, 100][station.danger] ?? 0
    if (dangerCost > 0) lines.push({ label: `Surtaxe zone (${['','risquée','dangereuse','guerre'][station.danger]})`, amount: dangerCost })
  }

  if (gs.faction && gs.faction !== 'none') lines.push({ label: 'Cotisation faction', amount: 25 })

  if ((gs.addictionLevel ?? 0) > 0) lines.push({ label: 'Dépendance', amount: (gs.addictionLevel ?? 0) * 40 })
